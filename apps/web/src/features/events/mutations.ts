...
export function useCreateEvent() {
  ...
  return useMutation({
    mutationFn: (input: { input: CreateEventInput }) =>
      createEvent({ data: { input: input.input } }),
...
export function useUpdateEvent() {
  ...
  return useMutation({
    mutationFn: (input: { id: string; itemId: string; input: UpdateEventInput }) =>
      updateEvent({ data: { id: input.id, input: input.input } }),
    onSuccess: (_, variables) => {
